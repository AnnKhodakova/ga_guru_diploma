...
class TestIncorporationE(
    ...
    def test_with_digital_id(self, setup_browser):
        ...
        with allure.step("Выбираем c е-резиденством"):
            self.choose_have_eres()

        with allure.step("Начинаем инкорпорацию"):
            self.start_incorporation_e()
    ...
    @allure.tag("dev")
    @allure.feature("Incorporation")
    def test_with_extra_member(self, setup_browser):
        with allure.step("Открываем страницу сайнапа"):
            self.open()
        ...
        with allure.step("Добавляем второго мембера"):
            self.add_extra_member_with_e()
            self.extra_member_should_be_added()
            self.go_to_next_page()

        with allure.step("Проверяем тексты на странице оплаты"):
            self.should_have_texts_with_extra_member_e()

        with allure.step("Выбираем план Стандарт"):
            self.choose_standard_plan()

        with allure.step("Оплачиваем в виджете чаржби"):
            self.pay_incorporation_with_e_in_chargebee()

        with allure.step("Подтверждаем оплату"):
            self.confirm_payment_with_e()

    @allure.severity(Severity.NORMAL)
    @allure.tag("dev")
    @allure.feature("Incorporation")
    def test_change_type_e_to_no_e(self, setup_browser):
        with allure.step("Открываем страницу сайнапа"):
            self.open()
        ...
        with allure.step("Меняем тип инкорпорации"):
            ...
            self.choose_incorporation_without_e()
